...
export default function PlayerPage(){
  ...
  const roleReveal = ()=>{
    ...
    el.animate([ {opacity:0, transform:'translateY(10px) scale(.98)'}, {opacity:1, transform:'translateY(0) scale(1)'} ], { duration:500, easing:'ease' });
    setTimeout(()=> el.animate([{opacity:1},{opacity:0}],{duration:400}).onfinish=()=> el.style.display='none', 2200);
  };
  ...
  return (
    <div className="wrap">
      ...
      <div id="reveal" className="card" style={{display:'none', textAlign:'center', fontSize:'1.2rem'}}>You are the <b>{role}</b></div>
      ...
      <div className="card">
        ...
        {phase==='night' && role && role!=='Villager' && (
          <div className="row">
      ...
      <div className="card">
        <h3>Story</h3>
        <div style={{fontSize:'1.1rem', lineHeight:1.6}}>{story||'Awaiting host...'}</div>
      </div>
    </div>
  );
}
